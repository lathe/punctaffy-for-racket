...
(require #/only-in racket/contract/base
  -> ->i any/c contract? contract-out list/c listof or/c
  rename-contract)
...
(require #/only-in lathe-comforts/struct
  auto-write define-imitation-simple-generics
...
(provide #/contract-out
  ...
  [dim-sys-dimlist-ref-and-call
    (->i
      (
        [ds dim-sys?]
        [lst (ds) (dim-sys-dimlist/c ds)]
        [i (ds) (dim-sys-dim/c ds)])
      [_ any/c])]
  ...
  [make-dim-sys-impl-from-max
    (->i
      (
        ...
        [dim-max-of-list (dim/c)
          (->i ([ds dim-sys?] [lsts (ds) (listof #/dim/c ds)])
            [_ (ds) (dim/c ds)])])
      [_ dim-sys-impl?])]
  ...
  [make-hyperstack
    (->i ([ds dim-sys?] [elems (ds) (dim-sys-dim/c ds)])
...
(define (make-hyperstack ds elems)
  (hyperstack ds #/dim-sys-dimlist-map elems #/fn elem
...
(define (hyperstack-push h elems-to-push)
  (dissect h (hyperstack ds rep)
  #/pushable-hyperstack #/dim-sys-dimlist-shadow ds
    (dim-sys-dimlist-zip-map ds
      elems-to-push
      (dim-sys-dimlist-chevrons rep)
      (fn elem rep-chevron
...
(define (hyperstack-pop h elems-to-push)
  ...
  #/list popped-barrier elem
    (hyperstack ds #/dim-sys-dimlist-shadow ds
      ...
      (dim-sys-dimlist-map ds elems-to-push #/fn elem
        (list 'pop elem rep))
      suspended-chevron)))
