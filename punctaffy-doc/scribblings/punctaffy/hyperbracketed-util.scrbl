...
@defform[
  ...
  @specsubform[atomic-form]{
    ...
    @itemlist[
      @item{Out of caution, this operation does not yet accommodate subforms represented by @racket[hash?] values. If interpolated expressions were allowed in hashes, their evaluation order would be hard to make guarantees about. If they were allowed in hash keys, expressions that were syntactically identical might end up counting as duplicate keys at read time or expansion time even if their run-time results would have been distinct. As for allowing interpolated expressions to appear in hash entries' associated values, there isn't a consistent precedent; Racket's @racket[quasiquote] seems to support @racket[unquote] there, but @racket[syntax] and @racket[quasisyntax] leave that location alone, processing neither template variables nor @racket[unsyntax]. To ensure that we don't implement a behavior prematurely that turns out to be surprising in practice, we don't allow hashes to appear as subforms here.}
