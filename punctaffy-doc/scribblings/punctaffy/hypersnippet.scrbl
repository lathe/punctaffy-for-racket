...
@defproc[
  ...
  Some of the lawfulness is associativity in a bird's-eye-view form: If all the @racket[hv-to-splice] results are selected suffixes, and if the prefix is content-free (in the sense that it can be converted to a shape by @racket[snippet-sys-snippet->maybe-shape]), then the result is the same as performing multiple concatenations to concatenate the suffixes with each other. In this sense, content-free hypersnippets are like concatenation operations in their own right, possibly like the composition cells of an @tech{opetopic} higher-dimensional weak category in category theory.
}
